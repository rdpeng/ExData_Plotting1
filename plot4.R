...
drawplot4 <- function(dataset){
        ...
        par(mfrow = c(2,2))
        with(dataset, plot(DateTime, Global_active_power, ylab = "Global Active Power", ylim = c(0, 8),type = "l"))
        with(dataset, plot(DateTime, Voltage, ylab = "Voltage", xlab = "datetime",type = "l"))
        with(dataset, plot(DateTime, Sub_metering_1,type = "l", ylab = "Energy sub metering"))
