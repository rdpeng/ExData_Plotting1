...
par(mfrow=c(2,2))
plot(1:length(hpc_to_use$Time),hpc_to_use$Global_active_power,"l",xaxt="n",xlab=NA,ylab="Global Active Power")
axis(1,at=c(1,0.5*length(hpc_to_use$Time), length(hpc_to_use$Time)),c("Thu","Fri","Sat"))
plot(1:length(hpc_to_use$Time),hpc_to_use$Voltage,"l",xaxt="n",xlab="datetime",ylab="Voltage")
axis(1,at=c(1,0.5*length(hpc_to_use$Time), length(hpc_to_use$Time)),c("Thu","Fri","Sat"))
...
plot(1:length(hpc_to_use$Time),hpc_to_use$Global_reactive_power,"l",xaxt="n",xlab="datetime",ylab="Global_reactive_power")
axis(1,at=c(1,0.5*length(hpc_to_use$Time), length(hpc_to_use$Time)),c("Thu","Fri","Sat"))
dev.off()
