...
with(db,
     {
       plot(db$Time,as.numeric(as.character(db$Global_active_power))
            ,type = "l"
            ,xlab = ""
            ,ylab = "Global Active Power") 
       
       plot(db$Time,as.numeric(as.character(db$Voltage))
            ,type = "l"
            ,xlab = "datetime"
            ,ylab = "Voltage")
       
       plot(db$Time,db$Sub_metering_1
            ,type = "n"
            ,xlab = ""
       ...
       plot( db$Time
             ,as.numeric(as.character(db$Global_reactive_power))
             ,type = "l"
             ,xlab = "datetime"
...
dev.copy(png, filename = here::here("exdata_consumption","plot4.png"))
dev.off()
