...
###Open Reshape2-library in order to be able to use melt() to put the three Sub_metering variables into one new variable. This is necessary, or at least is my solution, for getting all three Sub_metering variables into one graph (can be skipped if you've already run Plot3.R)
library(reshape2)
...
###Construct plot4 using BASE plotting system in R and write it to a png-file in the same folder where the dataset sits
...
par(mfrow = c(2, 2)) 
with(df, plot(df$Datetime2, df$Global_active_power, type="line", xlab="", ylab="Global Active Power"))
with(df, plot(df$Datetime2, df$Voltage, type="line", xlab="datetime", ylab="Voltage"))
...
with(df, plot(df$Datetime2, df$Global_reactive_power, type="line", xlab="datetime", ylab="Global_reactive_power"))
dev.off()
