...
# Creating and saving the graph as plot4.png
## Setting the graphical parameters to include a 2 by 2 layout with adjusted margins to allow the legend in the bottom left graph to fit properly
par(mfrow = c(2,2),mar=c(4,4,2,1))
## The sequence in plotting the four graphes starts on the top row left side, than top row right side, then second row left side and finally second row right side
### Top left graph
with(read3,plot(Date_Time,Global_active_power, type="l", xlab="", ylab="Global Active Power"))
### Top right graph
with(read3,plot(Date_Time,Voltage,xlab="datetime",ylab="Voltage",type="l"))
### Bottom left graph
...
legend("topright",bty="n",cex=0.7,lwd=0.5,col=c("black", "red","blue"),legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
### Bottom right graph
with(read3,plot(Date_Time,Global_reactive_power,xlab="datetime",type="l"))
